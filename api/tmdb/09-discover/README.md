...
|---|---|---|---|---|
| `certification` | query | string | ไม่ | use in conjunction with `region` |
| `certification.gte` | query | string | ไม่ | use in conjunction with `region` |
| `certification.lte` | query | string | ไม่ | use in conjunction with `region` |
| `certification_country` | query | string | ไม่ | use in conjunction with the `certification`, `certification.gte` and `certification.lte` filters |
| `include_adult` | query | boolean | ไม่ | - |
| `include_video` | query | boolean | ไม่ | - |
| `language` | query | string | ไม่ | - |
| `page` | query | integer | ไม่ | - |
| `primary_release_year` | query | integer | ไม่ | - |
| `primary_release_date.gte` | query | string | ไม่ | - |
| `primary_release_date.lte` | query | string | ไม่ | - |
| `region` | query | string | ไม่ | - |
| `release_date.gte` | query | string | ไม่ | - |
| `release_date.lte` | query | string | ไม่ | - |
| `sort_by` | query | string | ไม่ | - |
| `vote_average.gte` | query | number | ไม่ | - |
| `vote_average.lte` | query | number | ไม่ | - |
| `vote_count.gte` | query | number | ไม่ | - |
| `vote_count.lte` | query | number | ไม่ | - |
| `watch_region` | query | string | ไม่ | use in conjunction with `with_watch_monetization_types ` or `with_watch_providers ` |
| `with_cast` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_companies` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_crew` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_genres` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_keywords` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_origin_country` | query | string | ไม่ | - |
| `with_original_language` | query | string | ไม่ | - |
| `with_people` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_release_type` | query | integer | ไม่ | possible values are: [1, 2, 3, 4, 5, 6] can be a comma (`AND`) or pipe (`OR`) separated query, can be used in conjunction with `region` |
| `with_runtime.gte` | query | integer | ไม่ | - |
| `with_runtime.lte` | query | integer | ไม่ | - |
| `with_watch_monetization_types` | query | string | ไม่ | possible values are: [flatrate, free, ads, rent, buy] use in conjunction with `watch_region`, can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_watch_providers` | query | string | ไม่ | use in conjunction with `watch_region`, can be a comma (`AND`) or pipe (`OR`) separated query |
| `without_companies` | query | string | ไม่ | - |
| `without_genres` | query | string | ไม่ | - |
| `without_keywords` | query | string | ไม่ | - |
| `without_watch_providers` | query | string | ไม่ | - |
| `year` | query | integer | ไม่ | - |
#### Response 200
...
- `results` — array — 
  ...
  - `backdrop_path` — string —  (ตัวอย่าง: `"/8YFL5QQVPy3AgrEQxNYVSgiPEbe.jpg"`)
  - `genre_ids` — array — 
  ...
  - `popularity` — number —  (ตัวอย่าง: `9272.643`)
  ...
  - `vote_count` — integer —  (ตัวอย่าง: `1856`)
- `total_pages` — integer —  (ตัวอย่าง: `38020`)
- `total_results` — integer —  (ตัวอย่าง: `760385`)
...
| ชื่อ | ตำแหน่ง | ชนิด | จำเป็น | คำอธิบาย |
|---|---|---|---|---|
| `air_date.gte` | query | string | ไม่ | - |
| `air_date.lte` | query | string | ไม่ | - |
| `first_air_date_year` | query | integer | ไม่ | - |
| `first_air_date.gte` | query | string | ไม่ | - |
| `first_air_date.lte` | query | string | ไม่ | - |
| `include_adult` | query | boolean | ไม่ | - |
| `include_null_first_air_dates` | query | boolean | ไม่ | - |
| `language` | query | string | ไม่ | - |
| `page` | query | integer | ไม่ | - |
| `screened_theatrically` | query | boolean | ไม่ | - |
| `sort_by` | query | string | ไม่ | - |
...
| `with_genres` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_keywords` | query | string | ไม่ | can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_networks` | query | integer | ไม่ | - |
| `with_origin_country` | query | string | ไม่ | - |
| `with_original_language` | query | string | ไม่ | - |
| `with_runtime.gte` | query | integer | ไม่ | - |
| `with_runtime.lte` | query | integer | ไม่ | - |
| `with_status` | query | string | ไม่ | possible values are: [0, 1, 2, 3, 4, 5], can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_watch_monetization_types` | query | string | ไม่ | possible values are: [flatrate, free, ads, rent, buy] use in conjunction with `watch_region`, can be a comma (`AND`) or pipe (`OR`) separated query |
| `with_watch_providers` | query | string | ไม่ | use in conjunction with `watch_region`, can be a comma (`AND`) or pipe (`OR`) separated query |
| `without_companies` | query | string | ไม่ | - |
| `without_genres` | query | string | ไม่ | - |
| `without_keywords` | query | string | ไม่ | - |
| `without_watch_providers` | query | string | ไม่ | - |
| `with_type` | query | string | ไม่ | possible values are: [0, 1, 2, 3, 4, 5, 6], can be a comma (`AND`) or pipe (`OR`) separated query |
#### Response 200
...
- `results` — array — 
  ...
  - `popularity` — number —  (ตัวอย่าง: `2684.061`)
  - `poster_path` — string —  (ตัวอย่าง: `"/ujlkQtHAVShWyWTloGU2Vh5Jbo9.jpg"`)
  - `vote_average` — integer —  (ตัวอย่าง: `5`)
  - `vote_count` — integer —  (ตัวอย่าง: `13`)
- `total_pages` — integer —  (ตัวอย่าง: `7414`)
- `total_results` — integer —  (ตัวอย่าง: `148265`)
